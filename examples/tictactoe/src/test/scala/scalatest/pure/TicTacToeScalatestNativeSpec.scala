...
class TicTacToeSpecNative extends FunSpec with Matchers {
  ...
  describe("Place Spec") {
    it("should not be possible to place more than one stone at the same place") {
      (reset *>
      place(X, (1, 1)) *>
      place(O, (1, 1))).runA(BoardState.empty) shouldBe Left(OccupiedPosition((1, 1)))
    }

    it("Placing outside of the board is error") {
      (reset *>
      place(X, (5, 5))).runA(BoardState.empty) shouldBe Left(NotInTheBoard((5, 5)))
    }

    it("Placing in the wrong turn") {
      (reset *>
      place(O, (1, 1))).runA(BoardState.empty) shouldBe Left(WrongTurn(O))
    }
  ...
  describe("Simulation behaviour") {
    it("Unfinished match") {
      (reset *>
      simulate((0, 0), (0, 1))((1, 0), (1, 1))).runA(BoardState.empty) shouldBe Left(NotEnoughMoves)
    }
